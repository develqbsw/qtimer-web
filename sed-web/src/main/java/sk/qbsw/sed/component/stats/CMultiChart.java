 ...
 * @author lobb
 *
 *         toto je pokus vytvorit multi chart tak aby mal stlpce "Stacked" tak
 *         ako MultiBarChart. Aby bola line v popredi --> nv.d3.js
 * 
 *         http://stackoverflow.com/questions/26373756/nvd3-stacked-bar-chart-plus-line-overlapped
 * 
 ...
 * 
 *         Narazil som na dva problemy 1. cisla na x-ovej osi nie su zarovnane k
 *         stlpcom 2. maximum na y-ovej osi dava zle
 * 
 *         MultiBarChart nema line preto som ho nemohol pouzit.
 *
 */
public class CMultiChart extends CPanel {
	...
	public CMultiChart(String id, CFeedbackPanel errorPanel) {
		super(id);
	...
	private void init(boolean refresh, AjaxRequestTarget target, TimeRanges range) {
		...
		final BarGraphData barGraphData = new BarGraphData(attendanceDurationWeek, attendanceDurationMonth, timeRange, sumAndAverageTimeWeek, sumAndAverageTimeMonth, sumToday);
	...
	public BarGraphData(List<CAttendanceDuration> attendanceDurationWeek, List<CAttendanceDuration> attendanceDurationMonth, TimeRanges timeRange,
	...
	public String getValue() {
		...
		dataLeft += "{ \"x\": " + attendanceDuration.get(i).getDay().get(Calendar.DAY_OF_MONTH) + " , \"y\":"
			+ BigDecimal.valueOf(attendanceDuration.get(i).getDuration())
		...
		dataRight += "{ \"x\": " + attendanceDuration.get(i).getDay().get(Calendar.DAY_OF_MONTH) + " , \"y\":"
			+ BigDecimal.valueOf(averageForTimeRange)
				.divide(BigDecimal.valueOf(1000L * 3600), 2, RoundingMode.HALF_UP).toString()
			+ "}";
	    }
		dataRight += "]";
		...
		private String getFormatedHoursFromMillis(Long milsec) {
			...
			return value.setScale(0, RoundingMode.DOWN) + ":"
					+ formatter.format(value.subtract(value.setScale(0, RoundingMode.DOWN)).multiply(BigDecimal.valueOf(60)).setScale(0, RoundingMode.HALF_UP));

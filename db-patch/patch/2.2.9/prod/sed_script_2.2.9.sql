ALTER TABLE t_user ADD COLUMN c_flag_jira_token_generation BOOLEAN NOT NULL DEFAULT TRUE;

-- View: public.v_employees_status

DROP VIEW public.v_employees_status;

CREATE OR REPLACE VIEW public.v_employees_status
 AS
 SELECT tmp.pk_id,
    tmp.c_client_id,
    tmp.c_emp_code,
   ...
   FROM ( SELECT doch.pk_id,
            ...
            doch.home_office AS c_home_office,
                CASE
                    WHEN doch.ts_outside IS FALSE AND doch.ts_home_office IS FALSE AND (doch.c_status = 'IN_WORK'::text OR doch.c_status = 'OUT_OF_WORK'::text AND (breaks.activity_id = ANY (ARRAY['-1'::integer::numeric, '-2'::integer::numeric, '-7'::integer::numeric]))) THEN true
                    ELSE false
           ...
           FROM ( SELECT u.pk_id,
             ...
             LEFT JOIN ( SELECT (( SELECT 'now'::text::date AS date))::timestamp without time zone AS datum,
                  ...
                  WHERE r.fk_request_type != 7 AND rt.pk_id = r.fk_request_type AND rs.pk_id = r.fk_status AND (r.fk_status = ANY (ARRAY[3::numeric])) AND (( SELECT 'now'::text::date AS date)) >= r.c_date_from AND (( SELECT 'now'::text::date AS date)) <= r.c_date_to) requests ON doch.pk_id = requests.fk_user_owner AND doch.tmpday = requests.datum
...
ALTER TABLE public.v_employees_status
    OWNER TO qtimer_adm;

GRANT SELECT ON TABLE public.v_employees_status TO qtimer_gw;
GRANT ALL ON TABLE public.v_employees_status TO qtimer_adm;
GRANT SELECT ON TABLE public.v_employees_status TO qtimer_preview;